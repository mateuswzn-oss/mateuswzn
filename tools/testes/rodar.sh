...
declare -a NOMES=(
  ...
  "16 ajustes · as seis áreas que não são coleção"
)
declare -a CMDS=(
  ...
  "for a in profile settings college reports focus calendar; do node tools/testes/16-ajustes.mjs \$a || exit 1; done; for a in profile settings college reports focus calendar; do for t in claro escuro; do node tools/ds/quem-vence.mjs \$a \$t | sed -n 2p; done; done"
)
